...
function case_slide(device){
    ...
    for(var i=0;i<_case.data.length;i++){
        ...
        _html+="<div class='slide_item'><a href='"+_case.data[i].url+"' style='background-image: url("+_case.data[i].img+");'><div><span><span><p class='type'>"+_case.data[i].type+"</p><h3>"+_case.data[i].name+"</h3><p class='more'>更多作品</p></span></span></div></a></div>";
        if((i+1)%n==0){
            _html+="</div>";
        }
    }
